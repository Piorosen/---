...
bool SystemClass::Frame()
{
	...
	if (m_Input->IsKeyDown('W') || m_Input->IsKeyDown(VK_UP)) {
		data += 10;
	}
	if (m_Input->IsKeyDown('S') || m_Input->IsKeyDown(VK_DOWN)) {
		data += 20;
	}
	if (m_Input->IsKeyDown('A') || m_Input->IsKeyDown(VK_LEFT)) {
		data += 1;
	}
	if (m_Input->IsKeyDown('D') || m_Input->IsKeyDown(VK_RIGHT)) {
		data += 2;
	}if (m_Input->IsKeyDown('C')) {
		data += 200;
	}

	result = m_Graphic->Frame(data, D3DXVECTOR2((float)mouseX, (float)mouseY),m_cpu->GetCpuPercentage(), m_fps->GetFps(), m_timer->GetTime());
	if (!result)
		return false;
