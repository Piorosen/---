...
void Bullet::SetPosition(D3DXVECTOR3 player, D3DXVECTOR3 bullet)
{
	m_canFire = false;
	m_shoot = 0;
	player.y += screenHeight / SCREEN_Y / 2;
	playerPosition = player;
	bulletPosition = player;
	bulletPosition.x += 30;
	
	m = (playerPosition.y - bulletPosition.y) / (playerPosition.x - bulletPosition.x);
	gak = atan2(bulletPosition.y - playerPosition.y, bulletPosition.x - playerPosition.x) * 180 / 3.141592f;
...
bool Bullet::Render(ID3D11Device* device, ID3D11DeviceContext* context,	float bulletSpeed, float bulletSize, D3DXMATRIX world, D3DXMATRIX view, D3DMATRIX ortho)
{
	...
	// ¸Ê°úÀÇ ÃÑ¾Ë Ãæµ¹
	if (m_map->GetCollide({ GetBulletPosition().x, GetBulletPosition().y }, D3DXVECTOR2(1, 1))) {
		m_canFire = true;
		m_shoot = 1001;
		return false;
	}
